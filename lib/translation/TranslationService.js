...
class TranslationService extends EventEmitter {
    ...
    async processMessageBatch(channelId, messages, settings) {
        // 単一メッセージの場合は遅延を短縮
        const dynamicDelay = messages.length === 1 ? 10 : this.config.batch.delay || 100;
        ...
        for (const msg of messages) {
            ...
            await this.batchProcessor.addToBatch(channelId, timestampedMessage, options);
        }
